...
- **A screenshot already in Cloud Storage is sent as its `gs://` URI, not its bytes.** The
  ...
  turn and inline them; now `_material` builds a `StoredImage` and Vertex reads the object
...
- **What an answer means is the client's to say; this server owns the prompt's skeleton.**
  ...
  nobody described contributes nothing and logs `undescribed onboarding answer field=… value=…`.
  **That warning is the drift signal — alert on it.** `prompts=N` on the request and queue
  logs says how much traffic comes from a client that describes its answers; `0` at steady
